...
// PUBLIC_INTERFACE
export async function apiFetch(path, { method = "GET", body, headers = {}, auth = false, signal } = {}) {
  /**
...
export const Api = {
  ...
  health: () => apiFetch("/health").catch(() => "unavailable"),
  ...
  logout: () => apiFetch("/auth/logout", { method: "POST", auth: true }).finally(() => tokenStore.clear()),

  /** Profile endpoints */
  getProfile: () => apiFetch("/user/profile", { auth: true }),
  updateProfile: (data) => apiFetch("/user/profile", { method: "PUT", body: data, auth: true }),
  ...
  listRecipes: (params = {}) => {
    ...
    return apiFetch(`/recipes${suffix}`);
  },
  getRecipe: (id) => apiFetch(`/recipes/${encodeURIComponent(id)}`),
  createRecipe: (recipe) => apiFetch("/recipes", { method: "POST", body: recipe, auth: true }),
  updateRecipe: (id, recipe) => apiFetch(`/recipes/${encodeURIComponent(id)}`, { method: "PUT", body: recipe, auth: true }),
  deleteRecipe: (id) => apiFetch(`/recipes/${encodeURIComponent(id)}`, { method: "DELETE", auth: true }),

  /** Feedback */
  submitFeedback: (feedback) => apiFetch("/feedback", { method: "POST", body: feedback, auth: true }),
  listFeedback: () => apiFetch("/feedback", { auth: true }),
};
